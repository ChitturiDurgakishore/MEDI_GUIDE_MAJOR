<x-pharma-layout>
    <x-slot name="MainContent">
        <div class="container mt-4" style="max-width: 1000px; background: #f0f4f8; padding: 20px; border-radius: 8px;">
            ...
            <!-- Search Form -->
            <form method="GET" action="{{ url('/medicine-request') }}" class="mb-3 d-flex flex-column flex-sm-row gap-2" role="search">
                <input type="search" name="mobile" class="form-control flex-grow-1" placeholder="Search by Mobile Number"
                    value="{{ request('mobile') }}" aria-label="Search by mobile number" style="background: #fff; color: #222;">
                <button class="btn btn-outline-primary" type="submit" style="min-width: 120px;">Search</button>
            </form>
            ...
            <!-- Tab content -->
            <div class="tab-content border border-top-0 p-3 p-md-4" id="medicineRequestTabsContent" style="background: #e8edf3; color: #222; border-radius: 0 0 8px 8px;">
                <!-- Previous Requests Tab -->
                <div class="tab-pane fade show active" id="prev-requests" role="tabpanel" aria-labelledby="prev-requests-tab">
                    @if ($requests->isEmpty())
                        <p>No medicine requests found.</p>
                    @else
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover" style="min-width: 700px; background: #fff;">
                                    ...
                                    @foreach ($requests as $request)
                                        <tr>
                                            ...
                                            <td style="min-width: 150px;">
                                                <form action="{{ route('medicine-request.update-status', $request->id) }}" method="POST">
                                                    @csrf
                ...
                <div class="tab-pane fade" id="new-request" role="tabpanel" aria-labelledby="new-request-tab">
                    <form method="POST" action="{{ url('/medicine-request') }}"
                        class="mt-3 mt-md-4 p-3 p-md-4" style="max-width: 700px; background: #dce3ef; border-radius: 8px; color: #222;">
                        @csrf
                        <div class="mb-3">
                        ...
                        <button type="submit" class="btn btn-success btn-lg w-100">Submit Request</button>
                    </form>
                </div>
            </div>
        </div>
